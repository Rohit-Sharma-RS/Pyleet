...
function initEditor() {
    ...
    editor.setValue('# Paste a LeetCode URL above and click Fetch to get started!\n');
}
...
async function fetchProblemFromAPI(slug) {
    ...
    try {
        const response = await fetch(`/api/leetcode?slug=${encodeURIComponent(slug)}`);
        if (response.ok) {
    ...
    throw new Error('Could not fetch from LeetCode. Please deploy to Vercel or try again later.');
}
...
async function fetchProblem() {
    ...
    if (!slug) {
        urlError.textContent = 'Invalid LeetCode URL. Please use a URL like: https://leetcode.com/problems/two-sum/';
        return;
    }
    ...
    try {
        ...
        hideLoading();
        urlError.textContent = err.message || 'Failed to fetch problem. Please try again.';
...
function renderProblem(problem) {
    document.getElementById('problemTitle').textContent = `${problem.questionId}. ${problem.title}`;
...
function setupTestCases(problem) {
    ...
    const inputLines = (problem.exampleTestcases || '').split('\n').filter(line => line.trim() !== '');
...
function setupEditor(problem) {
    ...
    const pythonSnippet = snippets.find(s => s.langSlug === 'python3') || snippets.find(s => s.langSlug === 'python');

    if (pythonSnippet) {
        originalCode = pythonSnippet.code;
    } else {
        originalCode = '# Python code snippet not available for this problem.\n# Please write your solution here.\n\nclass Solution:\n    pass\n';
    }
...
async function runCode() {
    ...
    resultsBody.innerHTML = '<div class="loader-text" style="text-align:center;padding:20px;">Running tests...</div>';
        ...
        return `
            <div class="result-card ${r.passed ? 'pass' : 'fail'}">
                ...
                <div class="result-details">
                    ${Object.entries(testCases[i].inputs).map(([key, val]) => `
...
async function runSingleTest(userCode, methodName, params, testCase, index) {
    try {
        ...
        const argsStr = argValues.map(val => {
...
let historyDetailView = false; // track if showing detail view
...
function loadHistory() {
    ...
    const sorted = [...history].sort((a, b) => new Date(b.lastAttempt) - new Date(a.lastAttempt));
...
function loadFromHistory(slug) {
    ...
    document.getElementById('urlInput').value = `https://leetcode.com/problems/${slug}/`;
    toggleHistory();
